using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace MmPort
{
    public enum COMMUNICATION_TYPE:int
    {
        SerialPort=10000,
        Modbus=10001
    }


    public class MmUtils
    {

        public static bool[,] MBitVaue = new bool[16, 256];//定义输出线圈二维数组
        public static byte ucCRCHi = 0xFF;
        public static byte ucCRCLo = 0xFF;
        #region LRC校验
        public static string LRC(string strLRC)
        {
            int d_lrc = 0;
            string h_lrc = "";
            int l = strLRC.Length;
            for (int c = 0; c < l; c = c + 2)
            {
                string c_data = strLRC.Substring(c, 2);
                d_lrc = d_lrc + (Int32)Convert.ToByte(c_data, 16);
                //d_lrc = d_lrc + Convert.ToInt32(c_data);
            }
            if (d_lrc >= 255)
                d_lrc = d_lrc % 0x100;
            h_lrc = Convert.ToInt32(~d_lrc + 1).ToString("X");
            if (h_lrc.Length > 2)
                h_lrc = h_lrc.Substring(h_lrc.Length - 2, 2);
            return h_lrc;
        }
        #endregion

        #region  CRC校验
        private static readonly byte[] aucCRCHi = {
             
         };
        private static readonly byte[] aucCRCLo = {
             
         };
        public static void Crc16(byte[] pucFrame, int usLen)
        {
            int i = 0;
            ucCRCHi = 0xFF;
            ucCRCLo = 0xFF;
            UInt16 iIndex = 0x0000;

            while (usLen-- > 0)
            {
                iIndex = (UInt16)(ucCRCLo ^ pucFrame[i++]);
                ucCRCLo = (byte)(ucCRCHi ^ aucCRCHi[iIndex]);
                ucCRCHi = aucCRCLo[iIndex];
            }

        }
        #endregion

        #region 报文去除包头和校验，转换字节数组
        /// <summary>
        /// 报文转换成字节数组
        /// </summary>
        /// <param name="s"></param>
        /// <returns></returns>
        public static byte[] HexStringToByteArray(string s)
        {
            string Result = string.Empty;
            byte[] buffer = null;
            if (s != null && s.Length > 1)
            {
                string[] str = s.Trim().Split(' ');
                string[] Res = new string[str.Length];
                for (int i = 0; i < str.Length; i++)
                {
                    Res[i] = str[i];
                }
                for (int i = 0; i < Res.Length; i++)
                {
                    Result += Res[i] + " ";
                }

                string[] strArr = Result.Trim().Split(' ');
                buffer = new byte[strArr.Length + 1];
                for (int i = 0; i < strArr.Length; i++)
                {
                    buffer[i] = Convert.ToByte(strArr[i], 16);
                }
                buffer[strArr.Length] = Convert.ToByte(str[0], 16);
            }
            return buffer;
        }
        #endregion

        #region 字节变量赋值给bool型全局数组
        //字节变量返回8为二进制字符串
        public static string ByteToBinary(byte bValue)
        {
            string strTemp;
            int i, strLen;
            strTemp = System.Convert.ToString(bValue, 2);

            if (strTemp.Length < 8)
            {
                strLen = strTemp.Length;
                for (i = 0; i < 8 - strLen; i++)
                {
                    strTemp = "0" + strTemp;
                }
            }
            return strTemp;
        }
        //字节变量赋值给bool型全局数组
        public static void ByteToBArray(byte bValue, int iAddr, int pos)
        {
            string strBinary;
            int i;
            strBinary = ByteToBinary(bValue);
            for (i = 0; i < 8; i++)
            {
                if (strBinary.Substring(7 - i, 1) == "1")
                {
                    MBitVaue[iAddr, pos + i] = true;
                }
                else
                {
                    MBitVaue[iAddr, pos + i] = false;
                }
            }
            i = 10;

        }
        #endregion
    }
}
